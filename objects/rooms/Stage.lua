...
function Stage:new()
    ...
    self.player.color_red, self.player.color_green, self.player.color_blue = 0, 116, 217

    self.area:addGameObject('Actor', 900, 400)

    self.background_red, self.background_green, self.background_blue = random(255), random(255), random(255)
...
function Stage:shift_background_color()
    ...
    local shift_intensity = 5

    if color_to_shift == 1 then self.background_red = self.background_red + random(-shift_intensity, shift_intensity)
    elseif color_to_shift == 2 then self.background_green = self.background_green + random(-shift_intensity, shift_intensity)
    else self.background_blue = self.background_blue + random(-shift_intensity, shift_intensity)
    end

    if self.background_red > 255 then self.background_red = 255
    elseif self.background_red < 0 then self.background_red = 0
    end

    if self.background_green > 255 then self.background_green = 255
    elseif self.background_green < 0 then self.background_green = 0
    end

    if self.background_blue > 255 then self.background_blue = 255
